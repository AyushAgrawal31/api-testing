...
@allure.story("GET /api/v1/Announcement/{id}")
@allure.severity(allure.severity_level.CRITICAL)
def test_get_api_v1_announcement_id(request):

    client = APIClient()


    # Get real ID
    list_response = client.get("/api/v1/Announcement")
    ...
    sample_id = data[0]["id"]
    endpoint = "/api/v1/Announcement/{id}".replace("{id}", str(sample_id))
