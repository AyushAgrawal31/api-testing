...
@allure.story("GET /api/v1/BusinessUnit/{id}")
@allure.severity(allure.severity_level.CRITICAL)
def test_get_api_v1_businessunit_id(request):

    client = APIClient()


    # Get real ID
    list_response = client.get("/api/v1/BusinessUnit")
    ...
    sample_id = data[0]["id"]
    endpoint = "/api/v1/BusinessUnit/{id}".replace("{id}", str(sample_id))
